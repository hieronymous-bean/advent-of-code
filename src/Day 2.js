...
function isValidPassword(payload) {
	...
	for (var i = passwordRow.length - 1; i >= 0; i--) {
		const passwordObject = passwordRow[i].split(':')[i].toString();
		console.log(passwordObject);
		var ruleObject = (passwordRow[i].split(':'))[0];
		var rule = ruleObject.split(' ')[0];
		var letter = ruleObject.split(' ')[1];
		var ruleLow = rule.split('-')[0];
		var ruleHigh = rule.split('-')[1];

		console.log(passwordObject[i]);
	}
}